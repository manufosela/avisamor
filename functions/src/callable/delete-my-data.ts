  ...
  async (request) => {
    ...
    for (const groupId of groupsToCheck) {
      ...
      if (groupData.createdBy === uid) {
        ...
        if (otherMembers.empty) {
          ...
          const beacons = await db
            ...
            .where("groupId", "==", groupId)
            .get();
          for (const doc of beacons.docs) batch.delete(doc.ref);

          const alerts = await db
            .collection("alerts")
            .where("groupId", "==", groupId)
            .get();
          for (const doc of alerts.docs) batch.delete(doc.ref);

          const history = await db
            .collection("alertHistory")
            .where("groupId", "==", groupId)
            .get();
          for (const doc of history.docs) batch.delete(doc.ref);

          const subscription = await db
            .collection("subscriptions")
            .doc(groupId)
            .get();
          if (subscription.exists) batch.delete(subscription.ref);
        }
      }
      ...
      for (const alertDoc of alertsWithUser.docs) {
        ...
        const acceptedBy = alertData.acceptedBy as Array<Record<string, unknown>> | undefined;
        if (acceptedBy?.some((a) => a.uid === uid)) {
          const anonymized = acceptedBy.map((a) =>
            a.uid === uid ? { ...a, displayName: "[eliminado]", uid: "deleted" } : a
          );
